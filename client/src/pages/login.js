...
export default function LoginPage() {
  ...
  const { login, error, clearError, loading, isAuthenticated, user } = useAuthStore();
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 relative font-sans">
      ...
      <div className="max-w-md w-full bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-8 shadow-2xl relative">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-slate-100">Sign in to CollegeRAG</h2>
          <p className="text-slate-400 text-xs mt-1">Access the AI-Based RAG college chatbot</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              {/* Optional: forgot password link (not specified in spec.md, so keep it hidden/empty) */}
            </div>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-500">
